        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (currentMode) {
                case NORMAL_CONTROL:
                    ...
                    break;
                case POWERSHOTS:
                    // change curve, fire 1, change curve, fire 2, change curve, fire 3
                ...
                case ALIGN_TO_POINT:
                    ...
                    Vector2d robotFrameInput = new Vector2d(
                            -gamepad1.left_stick_y,
                            -gamepad1.left_stick_x
                    );
                    Vector2d fieldFrameInput = robotFrameInput.rotated(poseEstimate.getHeading());

                    // Difference between the target vector and the bot's position
                    Vector2d difference = targetPosition.minus(poseEstimate.vec().plus(shooterOffset.rotated(poseEstimate.getHeading())));
                    ...
                    double angleDiff = Angle.normDelta(theta - poseEstimate.getHeading());

                    if(angleDiff < Math.toRadians(30) && angleDiff > Math.toRadians(-25)) {
